% Analysis of 1fiber simulation with 100 replicates
...
%% FOD 
fod1_s = [0 0 1]; %%0,0,1; z-[0 0 1] [ sqrt(3)/2 0  1/2] [0.7071 0  0.7071] [1/2 0  sqrt(3)/2] [0.2588 0   0.9659]
theta0 = 0;
phi0 = 0;

%% saving path and folder name
folder_path = strcat(path_save,'simulation_review/','1fib','_lmax',num2str(lmax),'_b',num2str(b(1)),'_ratio',num2str(ratio(1)),'_n',num2str(n_sample),'_sig',num2str(sigma),'_SH8','/');
...
dirac_sh_lmax12 = weight(1)*dirac_sh1_lmax12;  %%SH representation
...
dirac_sh_lmax16 = weight(1)*dirac_sh1_lmax16;  %%SH representation
...
for rep = 1:N_rep
    ...
    if(nfib_SH_lmax8(rep)==1)
        ...
        angle1_SH_lmax8(rep,1) = ang111;
        
        %%%%%
        if(dot(fod_temp(1,:)',peak_pos_SH_final_lmax8(:,1))<0)
    ...
    if(nfib_sCSD_lmax8(rep)==1)
        ...
        angle1_sCSD_lmax8(rep,1) = ang111;
          
        %%%%%
        if(dot(fod_temp(1,:)',peak_pos_sCSD_final_lmax8(:,1))<0)
    ...
    if(nfib_sCSD_lmax12(rep)==1)
        ...
        angle1_sCSD_lmax12(rep,1) = ang111;
        
        %%%%%
        if(dot(fod_temp(1,:)',peak_pos_sCSD_final_lmax12(:,1))<0)
    ...
    if(nfib_sCSD_lmax16(rep)==1)
        ...
        angle1_sCSD_lmax16(rep,1) = ang111;
        
        %%%%%
        if(dot(fod_temp(1,:)',peak_pos_sCSD_final_lmax16(:,1))<0)
    ...
    if(nfib_SN(rep)==1)
        ...
        angle1_SN(rep,1) = ang111;
        
        %%%%%
        if(dot(fod_temp(1,:)',peak_pos_SN_final(:,1))<0)
...
%%%%%
rate_SH8 = sum(nfib_SH_lmax8==1)/N_rep;
under_SH8 = sum(nfib_SH_lmax8<1)/N_rep;
over_SH8 = sum(nfib_SH_lmax8>1)/N_rep;

rate_sCSD8 = sum(nfib_sCSD_lmax8==1)/N_rep;
under_sCSD8 = sum(nfib_sCSD_lmax8<1)/N_rep;
over_sCSD8 = sum(nfib_sCSD_lmax8>1)/N_rep;

rate_sCSD12 = sum(nfib_sCSD_lmax12==1)/N_rep;
under_sCSD12 = sum(nfib_sCSD_lmax12<1)/N_rep;
over_sCSD12 = sum(nfib_sCSD_lmax12>1)/N_rep;

rate_sCSD16 = sum(nfib_sCSD_lmax16==1)/N_rep;
under_sCSD16 = sum(nfib_sCSD_lmax16<1)/N_rep;
over_sCSD16 = sum(nfib_sCSD_lmax16>1)/N_rep;

rate_SN = sum(nfib_SN==1)/N_rep;
under_SN = sum(nfib_SN<1)/N_rep;
over_SN = sum(nfib_SN>1)/N_rep;
...
mean_angle1_SH_lmax8 = mean(angle1_SH_lmax8(nfib_SH_lmax8==1))*180/pi;
mean_angle1_sCSD_lmax8 = mean(angle1_sCSD_lmax8(nfib_sCSD_lmax8==1))*180/pi;
mean_angle1_sCSD_lmax12 = mean(angle1_sCSD_lmax12(nfib_sCSD_lmax12==1))*180/pi;
...
savefig(strcat(save_path,'1fib','_lmax',num2str(lmax),'_b',num2str(b(1)),'_ratio',num2str(ratio(1)),'_N',num2str(n_sample),'.fig'));
...
%%%%% latex table
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SH_input = round([rate_SH8 under_SH8 over_SH8 mean_angle1_SH_lmax8 mean_HD_SH_lmax8 sd_HD_SH_lmax8],4);
sCSD8_input = round([rate_sCSD8 under_sCSD8 over_sCSD8 mean_angle1_sCSD_lmax8 mean_HD_sCSD_lmax8 sd_HD_sCSD_lmax8],4);
sCSD12_input = round([rate_sCSD12 under_sCSD12 over_sCSD12 mean_angle1_sCSD_lmax12 mean_HD_sCSD_lmax12 sd_HD_sCSD_lmax12],4);
sCSD16_input = round([rate_sCSD16 under_sCSD16 over_sCSD16 mean_angle1_sCSD_lmax16 mean_HD_sCSD_lmax16 sd_HD_sCSD_lmax16],4);
SN_input = round([rate_SN under_SN over_SN mean_angle1_SN mean_HD_SN sd_HD_SN],4);

input.data = [SH_input;sCSD8_input;sCSD12_input;SN_input];

input.tableColLabels = {'correct','under','over', 'error 1', 'mean H-dist.', 'sd H-dist'};
...
input.dataFormat = {'%.2f',5,'%.4f',1}; % three digits precision for first two columns, one digit for the last
