...
fod1_s = [1 0 0]; %%0,0,1; z-[0 0 1] 0.9659 0 0.2588 [ sqrt(3)/2 0  1/2] [0.7071 0  0.7071] [1/2 0  sqrt(3)/2] [0.2588 0   0.9659] [sqrt(3)/2 0 1/2] [0.7660 0  0.6428]
...
save_path = strcat(path_save,'simulation_review/','2fib_sep',num2str(round(sep*180/pi,0)),'_lmax',num2str(lmax),'_b',...
    num2str(b(1)),'_ratio',num2str(ratio(1)),'_n',num2str(n_sample),'_sig',num2str(sigma),'_FOD',num2str(DTI_lmax),'ISO',num2str(ISOweight),'/');
mkdir(save_path);
...
for rep = 1:n_rep
    ...
    seed = rep*10+21;
    [DWI, theta_r, phi_r] = DWI_generate_FODaddISO(J_use, DTI_lmax, b, ratio, weight, theta0, phi0, sigma, ISOweight, half, Rotate, seed);
